...
// (bigger heading, underlined link) — still no real layout, blocks
// are meant to be stacked vertically in document order.
//
...
func (c *collector) flushParagraph() {
	if text := collapse(c.current.String()); text != "" {
		c.blocks = append(c.blocks, Block{Kind: BlockParagraph, Text: text})
	}
	c.current.Reset()
}
...
func (c *collector) walk(n *html.Node) {
	...
	if n.Type == html.ElementNode && headingTags[n.Data] {
		...
		if text := collapse(textOf(n)); text != "" {
			c.blocks = append(c.blocks, Block{Kind: BlockHeading, Text: text})
		}
		return
	}
	...
	if n.Type == html.ElementNode && n.Data == "a" {
		c.flushParagraph()
		if text := collapse(textOf(n)); text != "" {
			c.blocks = append(c.blocks, Block{Kind: BlockLink, Text: text, Href: attr(n, "href")})
		}
		return
	}
